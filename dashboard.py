...
def load_data():
    ...
    if (grp["risk_tier"]=="CRITICAL").sum() == 0: grp.loc[0,"risk_tier"] = "CRITICAL"
    if (grp["risk_tier"]=="HIGH").sum()     == 0: grp.loc[1,"risk_tier"] = "HIGH"
...
if regions_df is None or len(regions_df) == 0:
    st.error("CSV not found. Upload us_agri_support_agentic_dataset.csv via the Colab folder icon.")
    st.stop()
...
with left:
    ...
    fig_pie.update_layout(
        paper_bgcolor="rgba(0,0,0,0)", font=dict(color="#ffffff", size=12),
        showlegend=True,
        legend=dict(
            orientation="h",
        ...
        margin=dict(t=40, b=80, l=20, r=20), height=460,
        annotations=[dict(text="<b>9 USDA</b><br>Regions", x=0.5, y=0.5,
    ...
    fig_bar = go.Figure(go.Bar(
        x=bar_df["risk_score"], y=bar_df["region"], orientation="h",
        marker=dict(color=[REGION_COLORS.get(r,"#888") for r in bar_df["region"]],
    ...
    fig_bar.update_layout(
        ...
        margin=dict(t=10,b=10,l=10,r=80), height=360,
        xaxis=dict(title="Intervention Pressure Index (0-100)", gridcolor="#2e3a50",
                   tickfont=dict(color="#ffffff",size=11), range=[0,110],
                   titlefont=dict(color="#a0b4c8")),
        yaxis=dict(gridcolor="#2e3a50", tickfont=dict(color="#ffffff",size=12)),
        showlegend=False,
    )
...
with right:
    ...
    if row["risk_tier"] == "CRITICAL":
        sms = "[CRITICAL] " + rn + " (" + szn + "): Urgency " + rs + "/100. " + str(imm) + " producers need immediate intervention. Drought=" + dr + ", NDVI=" + nv + ", Repayment=" + rp + "%. Contact FSA TODAY."
        st.markdown(
            ...
            '<div class="int-row">💰 <b>FSA Emergency Loan:</b> <span style="color:#f85149;font-weight:700;">REQUIRED</span> — ' + str(apr) + ' producers need approval</div>'
        ...
        sms = "[HIGH RISK] " + rn + " (" + szn + "): Urgency " + rs + "/100. " + str(pri) + " producers flagged for priority deployment. Review within 48 hours."
        st.markdown(
            ...
            '<div class="int-row">💰 <b>FSA Loan Review:</b> <span style="color:#f0883e;font-weight:700;">RECOMMENDED</span> — ' + str(apr) + ' cases pending approval</div>'
        ...
        msg = "Active Monitoring — " + rn + ". " + str(act) + " producers under watch in " + si + ". Weekly field checks. Monitor NDVI (" + nv + ") and drought index (" + dr + "). Auto-escalate if urgency exceeds 42."
        st.info("🔵 " + msg)
        st.warning("📋 Contingency: Prepare input supply. If score rises above 42, escalate to Priority Deployment.")
    else:
        msg = rn + " — Stable. " + str(rtn) + " producers on routine monitoring. NDVI: " + nv + " · Repayment: " + rp + "%"
        st.success("🟢 " + msg)
    ...
    with s1:
        for label, val in [
            ("Avg Farm Size",   str(round(float(row.get("avg_farm_size_acres",0)),0))[:-2] + " acres"),
            ...
            st.markdown('<div class="stat-row">' + label + ': <span class="stat-val">' + val + '</span></div>',
                       unsafe_allow_html=True)
    with s2:
        for label, val in [
            ("Avg Loan Size",   "$" + str(round(float(row.get("avg_loan_usd",0))/1000,1)) + "K"),
            ("Rainfall",        str(round(float(row.get("avg_rainfall_mm",0)),0))[:-2] + " mm"),
            ("Repayment Rate",  rp + "%"),
    ...
    fig_radar.update_layout(
        polar=dict(
            bgcolor="rgba(0,0,0,0)",
            radialaxis=dict(visible=True, range=[0,1], gridcolor="#3a4a60",
    ...
    if raw_df is not None:
        ...
        with st.expander("View top at-risk producers — " + selected):
            ...
            cols = ["producer_id","state","county","primary_crop","farm_size_acres",
